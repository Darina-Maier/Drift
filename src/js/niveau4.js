...
export default class niveau4 extends Phaser.Scene {
  ...
  create() {
    ...
    this.time.delayedCall(300, () => {
      this.afficherJournalDeBord({
        planete: 'JOUSSET',
        gravite: '105 g',
        note: "Un désert aride à perte de vue. La pesanteur m'écrase — chaque pas est un effort. Et ces sphères étranges qui m'attirent vers elles... comme si la planète elle-même voulait me retenir.",
        touches: ['← → : déplacer    ESPACE : sauter']
      });
    });
  }
  ...
  finNiveau(player, teleporter) {
    ...
    if (this.niveauComplete) {
      ...
      if (!niveauxFinis.includes('niveau4')) {
        niveauxFinis.push('niveau4');
        this.game.registry.set('niveauxFinis', niveauxFinis);
      }
    }

    // retour tp menu principal
    if (this.musiqueNiveau4) this.musiqueNiveau4.stop();
    this.scene.start('pageprincipale');
  }
  ...
  afficherJournalDeBord(config) {
    ...
    // Fond vitré — ambiance désert beige/rose fushia
    const fond = this.add.graphics();
    fond.fillStyle(0x2e1a20, 0.85);
    fond.fillRoundedRect(x, y, largeur, hauteur, 12);
    fond.lineStyle(1, 0xff55aa, 0.6);
    ...
    reflet.fillStyle(0xff88cc, 0.08);
    reflet.fillRoundedRect(x + 4, y + 4, largeur - 8, 38, 8);
    reflet.setScrollFactor(0).setDepth(DEPTH);
    reflet[TAG] = true;

    const sLabel = { fontFamily: 'Orbitron', fontSize: '10px', color: '#ff55aa', letterSpacing: 2 };
    const sTitre = { fontFamily: 'Orbitron', fontSize: '17px', color: '#ffe0ee' };
    const sVal = { fontFamily: 'Orbitron', fontSize: '13px', color: '#ffaacc' };
    const sNote = {
      fontFamily: 'Orbitron', fontSize: '12px', color: '#ffbbdd',
      fontStyle: 'italic', wordWrap: { width: largeur - pad * 2 }
    };
    const sTouche = {
      fontFamily: 'Orbitron', fontSize: '11px', color: '#ffaacc',
      backgroundColor: '#2e1a20', padding: { x: 5, y: 2 }
    };
